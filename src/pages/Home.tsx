...
export function Home() {
  ...
  const generateCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div key={day} className={`h-12 sm:h-16 md:h-24 bg-gray-50/10 rounded-lg p-1 sm:p-2 ${event ? 'ring-2 ring-[#f14621]' : ''}`}>
          <span className="text-xs sm:text-sm font-medium text-gray-200">{day}</span>
          {event && (
            <div className="mt-1 hidden sm:block">
              <p className="text-xs font-semibold text-[#f14621] truncate">{event.title}</p>
              <p className="text-xs text-gray-400 hidden md:block">{event.time}</p>
            </div>
          )}
        </div>
      );
  ...
  return (
    <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-16 sm:mb-20">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {upcomingEvents.map((event) => (
                <div key={event.title} className="group relative overflow-hidden rounded-2xl">
                  ...
                  <div className="relative p-4 sm:p-6 flex flex-col h-full min-h-[250px] sm:min-h-[300px] justify-end">
            ...
            <div className="flex justify-center mt-8">
              <Link 
                to="/events"
                className="bg-[#f14621] text-white px-6 sm:px-8 py-2 sm:py-3 rounded-xl font-medium hover:bg-[#d13d1b] transition-colors backdrop-blur-sm bg-opacity-80 shadow-lg hover:shadow-xl transform hover:-translate-y-1"
              >
                Explore All Events
              </Link>
            </div>
      ...
      {/* Features Section */}
      <div className="py-16 sm:py-24 bg-[#eccec7]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
